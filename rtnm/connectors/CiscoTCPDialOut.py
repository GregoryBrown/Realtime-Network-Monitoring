...
class TelemetryTCPDialOutServer(TCPServer):
    def __init__(self, elasticsearch_server, elasticsearch_port, log_name):
        super().__init__()
        self.elastic_server = elasticsearch_server
        self.elastic_server_port = elasticsearch_port
        self.lock = Lock()
        self.log = self.init_log(f"{log_name}")

    async def get_index_list(self):
        try:
            http_client = AsyncHTTPClient()
            indices = []
            response = await http_client.fetch(f"http://{self.elastic_server}:{self.elastic_server_port}/*")
    ...
    async def post_data(self, data_to_post):
        try:
            ...
            url = f"http://{self.elastic_server}:{self.elastic_server_port}/_bulk"
    ...
    async def put_index(self, index):
        try:
            http_client = AsyncHTTPClient()
            url = f"http://{self.elastic_server}:{self.elastic_server_port}/{index}"
