...
def start_dial_out(address, port, elasticsearch_server, elasticsearch_port):
    log_name = f"{current_process().name}.log"
    tcp_server = TelemetryTCPDialOutServer(elasticsearch_server, elasticsearch_port, log_name)
    tcp_server.log.info(f"Starting listener on {address}:{port}")
    tcp_server.bind(port, address=address)
    tcp_server.start(0)
    IOLoop.current().start()


def start_dial_in_sub(address, port, username, password, elasticsearch_server, elasticsearch_port, batch_size, sub, worker_pool, index_list, lock, pem=None):
    log_name = f"{current_process().name}.log"
    ...
    es = ElasticSearchUploader(elasticsearch_server, elasticsearch_port, batch_size, es_log, index_list)
    with lock:
...
def main():
    ...
    if not dial_out["Address"] == "None":
        processes.append(Process(target=start_dial_out, args=(dial_out["Address"], dial_out["Port"],
                                                              dial_out["DatabaseIP"][0], dial_out["ES-Port"],), name="DialOutProcess"))
    elastic_lock = Lock()
    ...
    for client in clients:
        if not client == "cisco-dial-out":
            ...
            for sub in clients[client]["Subscriptions"]:
                if tls:
                    processes.append(Process(target=start_dial_in_sub, args=(clients[client]["Address"], clients[client]["Port"], clients[client]["Username"],
                                                                             clients[client]["Password"], clients[client]["DatabaseIP"][0], clients[client]["ES-Port"],
                                                                             clients[client]["BatchSize"], sub, worker_pool, index_list,
